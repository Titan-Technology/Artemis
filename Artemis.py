...
def parse(toks):
    i = 0
    while(i < len(toks)):
        if toks[i] + " " + toks[i+1][0:6] == "Display STRING" or toks[i] + " " + toks[i+1][0:3] == "Display NUM" or toks[i] + " " + toks[i+1][0:4] == "Display EXPR" or toks[i] + " " + toks[i+1][0:3] == "Display VAR":
...
def run():
    if argv[1].endswith('.hunt'):
        ...
        toks = lex(data)
        parse(toks)
    elif argv[1].endswith('.bow'):
        data = open_file(argv[1])
        toks = lex(data)
        parse(toks)
    elif argv[1].endswith('.arrow'):
        data = open_file(argv[1])
        toks = lex(data)
        parse(toks)
    elif argv[1].endswith('.titan'):
        data = open_file(argv[1])
        toks = lex(data)
        parse(toks)
    else:
